...
VoipExample::VoipExample(QWidget *parent) : QDialog(parent) { m_ui.setupUi(this); }
...
void VoipExample::on_startCallButton_clicked() {
  ...
  if (m_ui.audioGroupBox->isChecked()) {
    ...
    try {
      audiosrc = QGst::Bin::fromDescription("autoaudiosrc ! queue ! audioconvert ! audiorate ! audio/x-raw,rate=8000 "
  ...
  if (m_ui.videoGroupBox->isChecked()) {
    ...
    try {
      videosrc = QGst::Bin::fromDescription("videotestsrc is-live=true ! video/x-raw,width=320,height=240,framerate=15/1 "
...
void VoipExample::onRtpBinPadAdded(const QGst::PadPtr &pad) {
  ...
  try {
    ...
    if (pad->name().startsWith(QLatin1String("recv_rtp_src_1"))) {
      bin = QGst::Bin::fromDescription("rtpspeexdepay ! speexdec ! audioconvert ! autoaudiosink");
    } else { // recv_rtp_src_2_* -> session 2 - video
      bin = QGst::Bin::fromDescription("rtph264depay ! avdec_h264 ! videoconvert ! autovideosink");
    }
  } catch (const QGlib::Error &error) {
    qCritical() << error;
    qFatal("One ore more required elements are missing. Aborting...");
